...
DB_PATH_ALT = BASE_DIR / "data" / "jobs_d1.db"
...
# Covers: certified / certificate(s) / certification(s). The previous pattern
# (`certifi(ed|cation)\b`) silently failed on plurals -- "AWS Certifications"
# wouldn't match because the trailing "s" broke the \b boundary -- and never
# matched "certificate" at all, so both leaked into the gap list as if they
# were real technical skills.
CERT_PATTERN = re.compile(r"\bcertifi(?:ed|cate[sd]?|cation[s]?)\b", re.IGNORECASE)

# Best-effort backstop, NOT an exhaustive list. The primary defense against
# soft skills leaking into the gap list is the LLM extraction prompt itself
# (which is told explicitly to exclude them); this set only catches the
# required-skills side (parsed straight out of the DB's tech_stack column,
# which never goes through that prompt). Any soft-skill phrasing not listed
...
def _normalize_for_match(s: str) -> str:
    """Collapse common punctuation variants (dots, hyphens, underscores) to
    spaces so 'Node.js', 'Node-js', and 'node js' are treated as the same
    text. Used only by the no-LLM fallback matcher below -- it narrows, but
    doesn't eliminate, the gap between rigid exact-text matching and how
    people actually write skill names on a resume."""
    return re.sub(r"[.\-_]+", " ", s)


def skill_in_resume_text(skill: str, resume_lower: str) -> bool:
    """Direct/exact boundary-safe substring check -- used as the fallback
    path if Gemini is unavailable. This is inherently weaker than the LLM
    path: it can't recognize synonyms or abbreviations it wasn't told about
    (e.g. 'Postgres' vs 'PostgreSQL'), only punctuation variants."""
    skill_n = _normalize_for_match(skill)
    resume_n = _normalize_for_match(resume_lower)
    pattern = re.compile(
        r"(?<![a-z0-9])" + re.escape(skill_n).replace(r"\ ", r"\s+") + r"(?![a-z0-9])",
        re.IGNORECASE,
    )
    return bool(pattern.search(resume_n))
